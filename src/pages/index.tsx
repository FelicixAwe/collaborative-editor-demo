  ...
  useEffect(() => {
    ...
    elementsMap.forEach((yText: Y.Text, key: string) => {
      yText.observe((event: Y.YTextEvent) => {
        ...
        console.log("yText received updates: ", key);
    ...
    socketRef.current.on("update", ({ key, update }) => {
      console.log("Received an update of: ", key);
      ...
      if (ydocRef.current) {
        ...
        // console.log("Apply changes for " + key + " as " + yText.toString());
        const domElement = document.querySelector(`.${key}`);
        if (domElement) {
          domElement.textContent = yText.toString();
        }
